...
const extractSystem = `Voce e um analisador de perfil de tomada de decisao. Analise a d
{
"categoria": string,
...
const text = data.content.map(function(b) { return b.text || ''; }).join('');
const match = text.match(/\{[\s\S]*\}/);
if (!match) return res.status(500).json({ error: 'No JSON found', raw: text.substring(0,
return res.status(200).json({ result: JSON.parse(match[0]) });
};
} catch (error) {
